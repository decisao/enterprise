...
//          Result := False;
//          MsgError(Self,
//            'A senha digitada não confere. A operação não poderá ser confirmada.',
...
begin
    ...
    begin
      ...
      if cdsUserAccess.Locate('CODRESOURCE', TAction(Components[i]).Tag, []) then
       begin
         ...
         { verifico se a descricao é a mesma }
  ...
  { verifico se houve alterações e salvo }
  ...
  if SocketConnection.Connected then
   begin
     ...
     if midtier_servertype = 'U' then
        s := ReplaceStr(s, 'localhost', midtier_host + '/3051')
     else
        s := ReplaceStr(s, 'localhost', midtier_host);
     Result := s;
   end;
  if SocketConnection.Connected then
...
//  MsgError(Self, dbid, 'Assinante');
  ...
  if cdsUsuarios.Tag = 0 then
   begin
     dxStatusBar.Panels[3].Text := 'Lendo usuários em [' + midtier_host + '] ...';
     Application.ProcessMessages;
     ShowMsg('Conectando...');
     try
       cdsUsuarios.Open;
  ...
  if Result then
   if pos('V', GetAccess(act.Tag)) < 1 then
    begin
      ...
      MsgError(Format(
        'Olá, %s!' + #13#10#13#10 +
        'Lamento, mas você não possui privilégios suficientes' + #13#10 +
        'para acessar %s.',
...
begin
  MsgError(
    ...
    'decisao@decisao.net' + #13#10#13#10 +
    'versão 2017.21 (16/02/2017)',
    'Sobre...');
end;
...
begin
  if UserIn(Sender as TAction) then
   if cdsUsuarios.fieldByName('MUDAEMPRESA').AsString = 'S' then
    ...
    end else MsgError('Você não tem privilégios suficientes!', 'Erro');
end;
...
begin
  { pergunto se deseja sair - se não for atualização }
  CanClose := Pergunta(
...
begin
  { atualizo o relógio na barra de status }
  ...
  if UserIn(Sender as TAction) then
    try
      ...
      if Pergunta('O processo de Backup pode levar de alguns minutos ' +
        'até horas, dependendo do tamanho do banco de dados e da velocidade ' +
        'da sua rede.' + #13#10#13#10 +
        '> Não recomendamos efetuar backup através de redes sem fios (wireless). ' + #13#10#13#10 +
        '> Se o seu servidor estiver em outro local remoto, o processo pode ' +
        'durar mais tempo que o normal.' + #13#10#13#10#13#10 +
        'Se você entende a concorda com os termos, clique em "Sim".', 'Backup') = IDYES then
       begin
        ...
        { destino }
        stFBK := 'C:\' + dbid + FormatDateTime('YYYYMMDDHHNN', now) + '.FBK';

        { desconecto do servidor }
        SocketConnection.Close;

        { abro/executo o arquivo }
        opcoes := ' -b -v -t "' + caminho + '" "' + stFBK + '"' +
          ' -user ' + usuario + ' -pas ' + senha;

//        ShellExecute(0, 'open', PChar(GetAppDir + '\gbak.exe'), PChar(opcoes), nil, SW_SHOW);

        ExecAndWait(funcoes.GetAppDir + '\gbak.exe',
          ' -b -v -t "' + caminho + '" "' + stFBK + '"' +
          ' -user ' + usuario + ' -pas ' + senha,
          sw_ShowNormal);
        ...
        formPrincipal.MsgError('O backup foi concluído com sucesso! O arquivo ' +
          'com a cópia de reserva do seu banco de dados foi salvo em:' + #13#10#13#10 +
          stFBK, 'Backup OK!');
      ...
      on exception do
       begin
         HideMsg;
         MsgError(
           'Houve um erro ao efetuar o Backup. A operação não foi concluída.',
           'Erro');
       end;
    end;
...
begin
  if UserIn(Sender as TAction) then
    try
      WinExec(
        PAnsiChar(GetAppDir + '\NFPaulista.exe ' + GetServerPath),
         0);
    finally
      formPrincipal.HideMsg;
      UserOut;
...
begin
  if Pergunta(
    'A Redução Z encerra a emissão de cupons fiscais hoje. Deseja continuar?',
...
begin
  ...
  MsgError(
    'Número de série do ECF: ' + numero_serie,
    'Consulta');
end;
...
begin
  if Pergunta(
    'Essa operação cancela o último cupom ECF emitido. ' +
    'Deseja continuar?',
    'Cancela Cupom Emitido') = IDYES then
  ECFX.CancelarCupom;
end;
...
begin
  ...
  if pos('socket', E.Message) = 0 then
   ...
   begin
     SocketConnection.Close;
     Pronto('Conexão perdida. Tente novamente.');
   end;
end;
...
begin
  try
    ...
    if formConnConfig.ShowModal = mrOk then
      MsgError(
        'Configurações gravadas! Reinicie o sistema.',
        'Pronto!');
  finally
...
begin
  if Pergunta(
       'Esse procedimento reinicia todas as configurações ' +
       'pessoais. Continuar?',
       'Reiniciar configurações') = IDYES then
