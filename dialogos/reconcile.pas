...
uses
  ...
  cxLookAndFeelPainters, cxPC, cxControls, cxButtons, Menus, cxGraphics,
...
const
  ActionStr: array[TReconcileAction] of string = ('Pular', 'Abortar', 'Somar',
    'Curar', 'Cancelar', 'Atualizar');
  UpdateKindStr: array[TUpdateKind] of string = ('Modificação', 'Novo',
    'Deletado');
  SCaption = 'ERRO - %s';
  SUnchanged = '<Original>';
  SBinary = '(Binário)';
...
type
  ...
  public
    ...
    constructor CreateForm(DataSet: TDataSet; UpdateKind: TUpdateKind;
      Error: EReconcileError; c, ch: string);
  end;

function HandleReconcileError(DataSet: TDataSet;  UpdateKind: TUpdateKind;
  ReconcileError: EReconcileError; c, ch: string): TReconcileAction;
...
begin
  UpdateForm := TReconcileErrorForm.CreateForm(DataSet, UpdateKind, ReconcileError, c, ch);
  with UpdateForm do
  try
...
constructor TReconcileErrorForm.CreateForm(DataSet: TDataSet;
  UpdateKind: TUpdateKind; Error: EReconcileError; c, ch: string);
begin
  FDataSet := DataSet;
...
procedure TReconcileErrorForm.cxButton1Click(Sender: TObject);
begin
  modalresult := mrOk;
end;
